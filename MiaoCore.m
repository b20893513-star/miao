...
static BOOL MiaoTerminateBundle(NSString *bid) {
	...
	if (c) {
		...
		if ([c respondsToSelector:shared])
			svc = ((id (*)(id, SEL))objc_msgSend)(c, shared);
	}
	SEL term = NSSelectorFromString(@"terminateApplication:forReason:andReport:withDescription:");
	...
	typedef void (*BKSTerm)(CFStringRef, int, bool, CFStringRef);
...
/**
 Il pid di Safari secondo SpringBoard, 0 se non si riesce a saperlo.

 La richiesta di terminazione e' asincrona: torna subito, il processo muore
 dopo. Senza guardare il pid si cancellava BrowserState mentre Safari era
 ancora vivo, e uscendo lo riscriveva tale e quale.
 */
static pid_t MiaoSafariPid(void) {
	Class c = NSClassFromString(@"SBApplicationController");
	SEL shared = NSSelectorFromString(@"sharedInstance");
	if (!c || ![c respondsToSelector:shared]) return 0;
	id ctl = nil;
	@try { ctl = ((id (*)(id, SEL))objc_msgSend)(c, shared); }
	@catch (NSException *ex) { (void)ex; return 0; }
	SEL byId = NSSelectorFromString(@"applicationWithBundleIdentifier:");
	if (!ctl || ![ctl respondsToSelector:byId]) return 0;
	id app = nil;
	@try { app = ((id (*)(id, SEL, id))objc_msgSend)(ctl, byId, @"com.apple.mobilesafari"); }
	@catch (NSException *ex) { (void)ex; return 0; }
	if (!app) return 0;
	@try {
		id st = [app valueForKey:@"processState"];
		id pid = st ? [st valueForKey:@"pid"] : nil;
		if ([pid respondsToSelector:@selector(intValue)]) return (pid_t)[pid intValue];
	} @catch (NSException *ex) { (void)ex; }
	return 0;
}
 ...
 */
/// Vero quando l'ultimo scroll non e' nemmeno partito: senza una pagina piena
/// a schermo non c'e' niente da trascinare, e insistere e' tempo buttato.
static BOOL gScrollSkipped = NO;
...
static void MiaoScrollToThumb(NSInteger idx, NSInteger tries, void (^done)(BOOL ok, NSString *raw)) {
	MiaoJS(MiaoJSThumbAt(idx), ^(NSString *r) {
		...
		MiaoGestureScroll(dy, ^{
			/* Se il gesto non e' partito la pagina e' ferma, e riprovare non
			   cambia niente: nel log 35 scroll saltati di fila, poi OFF e un
			   recupero speso per nulla. Meglio dirlo subito a chi ha chiamato,
			   che sa rimettere il sito davanti. */
			if (gScrollSkipped) {
				done(visible, [NSString stringWithFormat:@"%@|scroll fermo", r]);
				return;
			}
...
/// iOS scrive il numero dentro la voce: "Chiudi tutte le 3 schede".
static NSArray<NSString *> *MiaoNamesCloseAllTabs(void) {
	return @[ @"Chiudi tutte le", @"Chiudi tutte", @"Close All Tabs",
			  @"Close All", @"Chiudi tutti i pannelli" ];
}
...
/// Tiene premuto un controllo nativo: il dito resta fermo, che sopra il mezzo
/// secondo iOS legge come pressione lunga e apre il menu.
static BOOL MiaoLongPressNode(MiaoAXNode *n, NSString *label) {
	if (!n) return NO;
	CGRect f = n.frame;
	CGPoint p = CGPointMake(CGRectGetMidX(f) + MiaoNudge(MIN(5, f.size.width * 0.15)),
							CGRectGetMidY(f) + MiaoNudge(MIN(5, f.size.height * 0.15)));
	NSString *why = nil;
	BOOL ok = MiaoUIKitSwipe(p, p, 0.95 + MiaoRnd() * 0.3, &why, nil);
	MiaoLog([NSString stringWithFormat:@"pressione lunga nativa %@ %@ (%@)",
		label, ok ? @"ok" : @"FAIL", why ?: @"-"]);
	return ok;
}
...
/// Schede con un URL secondo le API private: la scheda vuota non conta.
static NSInteger MiaoUrlTabCount(void) {
	NSInteger n = 0;
	for (id tab in MiaoTabList(MiaoBrowser()))
		if (MiaoTabURL(tab).length) n++;
	return n;
}

/**
 Le miniature della panoramica.

 Un nodo e' una miniatura se e' grande, ha un'etichetta (titolo o host della
 pagina) e non occupa quasi tutto lo schermo: cosi' si scarta il contenitore
 della griglia, che prima veniva scelto come "card piu' grande" e faceva
 finire la X nel vuoto.
 */
static NSArray<MiaoAXNode *> *MiaoTabCards(void) {
	CGRect b = UIScreen.mainScreen.bounds;
	CGFloat full = MAX((CGFloat)1, b.size.width * b.size.height);
...
static MiaoAXNode *MiaoCardCloseX(MiaoAXNode *card) {
	for (MiaoAXNode *c in MiaoAXFindAll(MiaoNamesClose())) {
		if (c.frame.size.width > 90 || c.frame.size.height > 90) continue;
		CGPoint mid = CGPointMake(CGRectGetMidX(c.frame), CGRectGetMidY(c.frame));
		if (!card || CGRectContainsPoint(CGRectInset(card.frame, -14, -14), mid)) return c;
	}
	return nil;
}
 ...
 */
static void MiaoCloseAllTabsInGrid(NSInteger tries, NSInteger stalls, void (^done)(BOOL ok));
static void MiaoCloseAllTabsHumanAttempt(NSInteger attempt, void (^done)(BOOL ok));

/// Apre la panoramica schede, se non ci siamo gia'.
static void MiaoOpenTabOverview(void (^done)(BOOL inGrid)) {
	if (MiaoInTabOverview()) {
		if (done) done(YES);
		return;
	}
	MiaoAXNode *btn = MiaoAXFind(MiaoNamesTabs());
	if (btn) MiaoTapNode(btn, @"schede-panoramica");
	else MiaoTapPt(MiaoPtTabs(), @"schede-panoramica-punto");
	MiaoAfter(MiaoBetween(1.0, 1.5), ^{ if (done) done(MiaoInTabOverview()); });
}

/**
 Chiude le miniature una alla volta.

 Conta le schede rimaste, non le X toccate: prima ogni tap valeva come una
 chiusura e il ciclo finiva convinto di aver ripulito tutto. Se due giri di
 fila non fanno scendere il numero, si smette invece di battere a vuoto.
 */
static void MiaoCloseAllTabsInGrid(NSInteger tries, NSInteger stalls, void (^done)(BOOL ok)) {
	if (MiaoTabsLookClosed()) {
		MiaoLog(@"chiudi griglia: schede finite");
		if (done) done(YES);
		return;
	}
	...
	NSUInteger before = cards.count;
	MiaoAXNode *card = cards.firstObject;
	MiaoAXNode *x = MiaoCardCloseX(card);
	BOOL tapped = NO;
	if (x) tapped = MiaoTapNode(x, @"chiudi-scheda-griglia");
	if (!tapped) {
		/* La X sta nell'angolo in alto a destra della miniatura. */
		CGPoint p = CGPointMake(CGRectGetMaxX(card.frame) - 22, card.frame.origin.y + 22);
		tapped = MiaoTapPt(p, @"chiudi-x-angolo");
	}
	if (!tapped) {
		MiaoLog(@"chiudi griglia: tap X fallito");
		if (done) done(NO);
		return;
	}
	MiaoAfter(MiaoBetween(0.5, 0.85), ^{
		NSUInteger after = MiaoTabCards().count;
		MiaoCloseAllTabsInGrid(tries - 1, after < before ? 0 : stalls + 1, done);
	});
}
...
static void MiaoCloseAllTabsHumanAttempt(NSInteger attempt, void (^done)(BOOL ok)) {
	...
	/* In panoramica il menu "Chiudi tutte" non c'e': la barra ha solo + e Fine.
	   Dopo tre tentativi col menu si passa comunque alle miniature. */
	...
	MiaoAfter(MiaoBetween(1.1, 1.7), ^{
		...
		MiaoAfter(MiaoBetween(0.9, 1.4), ^{
			...
			MiaoAfter(MiaoBetween(1.2, 1.8), ^{
				if (MiaoTabsLookClosed()) {
					MiaoLog(@"chiudi tutte: menu ha ripulito");
					if (done) done(YES);
					return;
				}
...
static void MiaoActCloseAllTabs(void (^done)(BOOL ok)) {
	...
	/* Le API dicono spesso "zero schede" mentre a schermo ce ne sono: si crede
	   allo zero solo se anche webview e miniature sono d'accordo. */
	MiaoAfter(0.5, ^{
		if (MiaoTabsLookClosed()) {
			finish();
			return;
...
static void MiaoCloseAdTabNative(void (^done)(BOOL ok)) {
	...
	/* Gia' in panoramica: non ritoccare Schede (riaprirebbe / confonderebbe). */
	if (MiaoInTabOverview()) {
...
static void MiaoBackFromHijackedTab(void (^done)(BOOL ok)) {
	...
	MiaoGoBackHuman(^(BOOL back) {
		...
		MiaoAfter(MiaoHumanDelay(1.0, 0.7), ^{
			...
			/* Indietro non ha storia da percorrere: l'ad non ha preso la nostra
			   scheda, e' una scheda nuova. Allora si chiude, e Nox torna davanti
			   senza ricaricarsi. Riaprire la home qui era il giro infinito: nel
			   log 23 openURL di fila, uno per ogni tentativo, e ogni home nuova
			   ri-arma il popunder. */
			MiaoLog(@"scheda rubata: niente storia indietro, chiudo l'ad");
			MiaoCloseAdsHuman(^(BOOL front) { if (done) done(front); });
		});
	});
}
...
static void MiaoEnsureSiteFront(void (^done)(BOOL ok)) {
	...
	MiaoCloseAdTabNative(^(BOOL nativeOk) {
		...
		/* Prima la scheda Nox: sull'ad l'Indietro puo' riportare la pagina che
		   l'ha aperto, cioe' una ricarica del sito e un altro giro di pop. */
		MiaoReturnToSiteTab(^(BOOL onSite) {
			if (onSite) {
				MiaoLog(@"front ok via scheda Nox");
				if (done) done(YES);
				return;
			}
			MiaoGoBackHuman(^(BOOL back) {
...
static void MiaoCloseAdsHuman(void (^done)(BOOL siteFront)) {
	...
	/* Qui NON si guarda se esiste una scheda Nox: Safari scarica la webview
	   delle schede in secondo piano e le API schede su iOS 16 spesso tornano
	   vuote, quindi "non c'e' nessuna scheda Nox" era falso quasi sempre. Su
	   quella falsa risposta si finiva a riaprire la home a ogni giro. */

	/* Si torna sulla scheda Nox che c'e' gia': ricaricarla ri-arma il popunder,
	   ed e' proprio quello che vogliamo evitare. La home si riapre solo se dopo
	   la chiusura non e' rimasta davvero nessuna pagina del sito. */
	void (^finish)(void) = ^{
		MiaoReturnToSiteTab(^(BOOL ok) {
			...
			if (!MiaoSiteTabExists()) {
				MiaoLog(@"ads chiuse ma nessuna pagina Nox: riapro la home");
				MiaoToast(@"Riapro il sito");
				MiaoOpenURL(MiaoHomeURL());
				MiaoAfter(3.2, ^{
					if (done) done(MiaoSiteIsFront() && !MiaoInTabOverview());
...
/**
 ...
 il video. openURL home e' vietato a meta' run (nuova tab, home ricaricata e
 popunder in loop: era il giro "ad, torno al sito, ad" senza fine).
 */
static void MiaoRunContinueToVideo(NSString *why) {
...
static void MiaoRunStartOnSite(void) {
	MiaoEnsureSiteFront(^(BOOL front) {
		...
		if (!u.length || !MiaoIsSiteURL(u)) {
			/* Il recupero chiude l'ad e riprende la scheda Nox; la home la
			   riapre solo se non ne e' rimasta nessuna. Decidere qui in base a
			   MiaoSiteTabExists() portava a ricaricare una home che c'era. */
			MiaoRunContinueToVideo(@"pagina estranea davanti");
			return;
		}
		start();
	});
...
/// Stesso schema per la chiusura schede: Safari posta `tabsdone` quando ha
/// finito col menu o con le miniature. Il timer fisso di prima killava Safari
/// a gesto iniziato, e lo stato su disco restava quello vecchio.
static void (^gTabsDoneBlock)(void) = nil;
static BOOL gTabsDoneListening = NO;
static NSInteger gTabsDoneGen = 0;

static void MiaoTabsDoneListen(void) {
	if (gTabsDoneListening || !MiaoIsSB()) return;
	gTabsDoneListening = YES;
	int token = 0;
	notify_register_dispatch("com.noxlab.miao.tabsdone", &token, dispatch_get_main_queue(), ^(int t) {
		(void)t;
		void (^b)(void) = gTabsDoneBlock;
		gTabsDoneBlock = nil;
		if (b) b();
	});
}

static void MiaoAwaitTabsDone(NSTimeInterval timeout, void (^done)(BOOL fromSafari)) {
	MiaoTabsDoneListen();
	NSInteger gen = ++gTabsDoneGen;
	gTabsDoneBlock = ^{
		if (gen != gTabsDoneGen) return;
		gTabsDoneGen++;
		if (done) done(YES);
	};
	MiaoAfter(timeout, ^{
...
/// Anche se il kill e' fallito, il ciclo dopo deve comunque chiudere le
...
static void MiaoRunCycle(NSInteger idx, NSInteger total, void (^done)(void)) {
	...
	/* Da app appena avviata serve piu' respiro: il tweak dentro Safari deve
	   caricarsi e registrare i suoi canali, e un comando spedito prima finisce
	   nel file e resta la' fino al prossimo poll. */
	BOOL cold = gColdStart;
	BOOL fresh = gNeedFresh;
	gColdStart = NO;
	gNeedFresh = NO;
